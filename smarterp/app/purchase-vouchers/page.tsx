    ...
    <DashboardLayout>
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-white sm:text-3xl">Purchase Vouchers</h1>
            <p className="text-sm text-zinc-400">Track and view inward inventory supplies and outstanding supplier balances</p>
          </div>
          <Link
              ...
              <tbody className="divide-y divide-zinc-800 text-zinc-300">
                {vouchers.map((voucher, i) => (
                  ...
                  >
                    ...
                    <td className="p-4 font-mono text-zinc-500 text-xs truncate max-w-[120px]" title={voucher.id}>
                      {voucher.id}
                    </td>
          ...
          <div className="w-full max-w-2xl bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-2xl animate-in fade-in-50 zoom-in-95 duration-150">
            ...
            <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-6 bg-zinc-950 p-4 rounded-lg border border-zinc-800">
                <div>
                  <h4 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">Inward From:</h4>
                  <p className="font-bold text-white">{viewVoucher.suppliers?.name}</p>
                  <p className="text-sm text-zinc-400">{viewVoucher.suppliers?.mobile || 'No Mobile'}</p>
                  <p className="text-sm text-zinc-400 mt-1">{viewVoucher.suppliers?.address || 'No Address'}</p>
                </div>
                <div className="text-right">
                  <h4 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">Voucher Info:</h4>
                  ...
                  <p className="text-sm text-zinc-400 mt-1">Status: <span className="text-emerald-400 font-bold bg-emerald-500/10 px-2 py-0.5 rounded text-xs">Received</span></p>
                </div>
              </div>
